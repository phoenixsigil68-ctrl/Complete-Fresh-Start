...
function PaperDisplay({
  ...
  paper: NonNullable<CreatePaperState["data"]>;
  onPrint: () => void;
}) {
      ...
      <Card className="mt-6 print:shadow-none print:border-none">
        <CardHeader className="flex flex-row justify-between items-start">
          <div>
            ...
            <CardDescription>
              {t("paperGenerator.totalMarks", { marks: paper.totalMarks })}
            </CardDescription>
          </div>
        ...
        <CardContent className="space-y-8">
          ...
          <section>
            <h3 className="text-lg font-semibold border-b pb-2 mb-4">
              {t("paperGenerator.mcqs")} (5 {t("paperGenerator.marks")})
            </h3>
          ...
          <section>
            <h3 className="text-lg font-semibold border-b pb-2 mb-4">
              {t("paperGenerator.shortAnswer")} (10 {t("paperGenerator.marks")})
            </h3>
            <div className="space-y-4">
          ...
          <section>
            <h3 className="text-lg font-semibold border-b pb-2 mb-4">
              {t("paperGenerator.longAnswer")} (10 {t("paperGenerator.marks")})
            </h3>
            <div className="space-y-4">
          ...
          <div className="text-center font-bold text-primary pt-4">
            {t("paperGenerator.endOfPaper")}
          </div>
        </CardContent>
      </Card>
    </div>
...
export function PaperGenerator({
  ...
  const handleGenerate = async () => {
    setIsLoading(true);
    setState(null);
    const result = await createPaperAction(
      chapter.content,
      grade.name,
      subject.name,
      language
